...
const Navbar = () => {
  ...
  const navLinks = (
    <>
      <li><NavLink to="/" end className={({ isActive }) => isActive ? "underline underline-offset-4 text-orange-300" : undefined}>Home</NavLink></li>
      <li><NavLink to="/fridge" className={({ isActive }) => isActive ? "underline underline-offset-4 text-orange-300" : undefined}>Fridge</NavLink></li>
      {user && (
        <>
  ...
  return (
    <div className={`navbar fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isTop ? 'bg-transparent' : 'bg-gradient-to-r from-orange-300 to-orange-600 shadow-md'
    }`}>
      <ToastContainer />
      ...
      <div className="navbar-center hidden lg:flex">
        <ul className="menu menu-horizontal px-1 text-green-100 font-medium">
          {navLinks}
        </ul>
      </div>
      ...
      <div className="navbar-end flex items-center gap-4">
        {!user ? (
          <div className="flex gap-3">
            <Link to="/auth/login">
              <button className="btn bg-orange-500 hover:bg-orange-300 text-white font-medium rounded-full">Login</button>
            </Link>
            <Link to="/auth/register">
              <button className="btn border border-white hover:bg-white hover:text-orange-500 text-orange-300 font-medium rounded-full">Register</button>
            </Link>
          </div>
        ) : (
          <div className="flex items-center gap-4">
            <button
              ...
              className="btn bg-orange-500 hover:bg-orange-300 text-white font-medium rounded-full"
            >
              Logout
            </button>
